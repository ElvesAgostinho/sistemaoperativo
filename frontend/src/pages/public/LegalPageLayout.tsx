...
export default function LegalPageLayout({ titulo, ultimaAtualizacao, children }: LegalPageLayoutProps) {
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#FAFCFB', fontFamily: FONT_BODY, color: INK }}>
            <header style={{ backgroundColor: '#0F1917', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '14px' }}>
                <a href="/" style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'white', textDecoration: 'none' }}>
                    <LogoMark size={32} />
                    <span style={{ fontFamily: FONT_DISPLAY, fontSize: '18px', fontWeight: 800 }}>BusinessOS</span>
                </a>
            ...
            <main style={{ maxWidth: '760px', margin: '0 auto', padding: '48px 24px 80px' }}>
                <h1 style={{ fontFamily: FONT_DISPLAY, fontSize: '30px', fontWeight: 800, margin: '0 0 6px 0', letterSpacing: '-0.01em' }}>{titulo}</h1>
                <p style={{ color: INK_MUTED, fontSize: '13.5px', margin: '0 0 28px 0' }}>Última atualização: {ultimaAtualizacao}</p>

                <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', backgroundColor: ACCENT_SOFT, border: `1px solid ${BORDER}`, borderRadius: '10px', padding: '16px 18px', marginBottom: '36px' }}>
...
export const secaoTitulo: CSSProperties = { fontFamily: FONT_DISPLAY, fontSize: '19px', fontWeight: 800, color: INK, margin: '32px 0 10px 0' };
